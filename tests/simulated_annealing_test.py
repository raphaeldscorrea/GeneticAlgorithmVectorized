""" Tests related to Simulated Annealing I """
import os
import pytest
import numpy as np
...
CALCULATE_FILES = [f for f in os.listdir('tests/simulated_annealing_tests/calculate/inputs')
                       if os.path.isfile(os.path.join('tests/simulated_annealing_tests/calculate/inputs', f))]
...
class TestClass(object):
    """
        Class with all tests for Simulated Annealing I
    """
    @staticmethod
    @pytest.mark.parametrize('instance', CALCULATE_FILES)
    def test_calculate_fitness_value(instance):
        
        instance = Instance('tests/simulated_annealing_tests/calculate/',
                            instance)
